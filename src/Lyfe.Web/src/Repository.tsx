...
export async function postRecord(url: string, token: string, body: Day | Exercise | Weight) {
  const response = await fetch(url, {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${token}`,
...
export async function patchRecord(url: string, token: string, body: Day | Exercise | Weight) {
  const response = await fetch(url, {
    method: "PATCH",
    headers: {
      "Authorization": `Bearer ${token}`,
